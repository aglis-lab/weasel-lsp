...
namespace wlsp
{
	...
	Capability::Capability(String method, JsonIO params, optional<JsonIO> result) : method(method),
																					params(params),
																					result(result){};
	...
	Capability::JsonIO::JsonIO(optional<function<void(JsonWriter &, any &)>> writer,
							   optional<function<ValueSetter(JsonHandler &, optional<any> &)>> reader) : writer(writer),
																										 reader(reader){};
		...
		{{// Writer
		  ...
		  [](JsonHandler &handler, optional<any> &data)
		  {
			  ...
			  return ValueSetter{
				  ...
				  [&handler, &params]()
				  {
					  auto *maker = new ObjectArrayMaker<WorkspaceFolder>(params.emplace<vector<WorkspaceFolder>>());
